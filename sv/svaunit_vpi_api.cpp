...
//			start_time);
  ...
  cb_assertion_info crt_callback_info(assertion_name, assertion_path, assertion_type, reason,
        ...
        while ((sva = vpi_scan(itr_sva)) != NULL) {
          ...
          if (exists == 0) {
            ...
            printf("Registering assertion: %s with type: %s\n",
                assertion_path.c_str(), assertion_type.c_str());

            //send information to SV
            create_assertion_dpi(get_char_from_string(assertion_name),
                get_char_from_string(assertion_path),
                get_char_from_string(assertion_type));

            put_callbacks_on_assertion(sva);
          }
        }
      }
    }
